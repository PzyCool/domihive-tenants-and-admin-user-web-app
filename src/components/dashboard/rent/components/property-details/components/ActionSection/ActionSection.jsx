...
const ActionSection = ({ propertyId, property, onBookInspection }) => {
  ...
  const handleBookInspection = () => {
    if (isBookable && isInterested && onBookInspection) {
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-3">
          <div className="flex items-center h-6">
            <input
              ...
              className="w-5 h-5 text-[#9f7539] bg-[#f8fafc] border-[#e2e8f0] rounded focus:ring-[#9f7539]/20 focus:ring-2 disabled:opacity-60"
              disabled={!isBookable}
            />
          </div>
          <div className="flex-1">
            <label
              ...
              className="action-label text-[#0e1f42] font-medium cursor-pointer select-none"
            >
              I like this property and I'm interested in proceeding
            </label>
            <p className="action-helper text-sm text-[#64748b] mt-1">
              {isBookable
        ...
        <div>
          <button
            ...
            className={`action-button w-full py-3 px-4 rounded-xl font-medium transition-all duration-300 ${
          ...
          {isInterested && isBookable && (
            <p className="action-helper text-sm text-[#64748b] mt-2 text-center">
        ...
        {/* Status Indicator */}
        <div className={`action-status p-3 rounded-lg ${isInterested && isBookable ? 'bg-green-50 border border-green-200' : 'bg-[#f8fafc] border border-[#e2e8f0]'}`}>
